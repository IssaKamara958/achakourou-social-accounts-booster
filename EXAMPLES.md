...
export function MyAccounts() {
  const { accounts, isLoading, deleteAccount, createAccount, updateAccount } = useSocialAccounts();
...
```typescript
import { SocialAccountsService, SocialPostsService, AIOptimizationService } from "@/lib/social";
...
async function optimizePost(caption: string, hashtags: string[]) {
  const recommendations = await AIOptimizationService.analyzePost(caption, hashtags, "instagram");
...
(async () => {
  ...
  if (accounts.length > 0) {
    ...
    const optimized = await optimizePost(post.caption!, post.hashtags);
  }
})();
```
...
async function refreshAccessToken(
  ...
  const newToken = await OAuthService.refreshToken(platform, account.refresh_token);
...
async function facebookExample(accessToken: string) {
  ...
  // Create post
  const newPost = await api.createPost(pages[0].id, "Hello Facebook! 👋");
...
async function instagramExample(accessToken: string, businessAccountId: string) {
  ...
  const post = await api.createPost("https://example.com/image.jpg", "Check this out!", "IMAGE");
...
async function aiOptimizationExample() {
  ...
  // Full analysis
  const analysis = await AIOptimizationService.analyzePost(caption, hashtags, "tiktok");
  ...
  const seoDesc = await AIOptimizationService.generateSEODescription(caption, "instagram");
  console.log("SEO:", seoDesc);

  // Generate hashtags
  const tags = await AIOptimizationService.generateHashtags(caption, "instagram", 20);
  console.log("Tags:", tags);
  // ['#innovation', '#technology', ...]

  // Generate viral title
  const title = await AIOptimizationService.generateViralTitle(caption, "tiktok");
  console.log("Title:", title);

  // Analyze sentiment
  const sentiment = await AIOptimizationService.analyzeSentimentAndCTA(caption, "facebook");
  ...
  const viralScore = await AIOptimizationService.predictViralPotential(caption, hashtags, "tiktok");
...
export function SocialDashboard() {
  ...
  useEffect(() => {
    if (selectedAccount && posts.length > 0) {
      const post = posts[0];
      AIOptimizationService.analyzePost(post.caption || "", post.hashtags, post.platform).then(
        setRecommendations,
      );
    }
