...
const MOCK_SCRIPT: Script = {
  ...
  content:
    "Voici comment les meilleurs créateurs africains utilisent l'IA pour tripler leur audience en 30 jours. Étape 1 : identifiez votre niche locale. Étape 2 : créez des hooks qui parlent à votre culture. Étape 3 : publiez aux heures de pointe (18h-22h heure de Dakar).",
...
function Generator() {
  ...
  async function generate() {
    ...
    if (!canUse("ai")) {
      toast.error(
        `Quota IA épuisé — ${quota.limit} générations gratuites/jour. Revenez demain ! 🇸🇳`,
      );
      return;
    }
    setBusy(true);
  ...
  function exportJson() {
    if (!script) return;
    const blob = new Blob([JSON.stringify({ topic, tone, niche, ...script }, null, 2)], {
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Génère HOOK · CONTENU · CTA + captions multi-plateforme.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl border border-destructive/40 bg-destructive/5 text-sm">
          ...
          <div>
            ...
            <span className="text-muted-foreground">
              {quota.limit} générations IA gratuites/jour. Renouvellement à minuit. 🇸🇳
            </span>
          </div>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <Card className="p-6 space-y-4 bg-card border-border lg:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {NICHES.map((n) => (
                <button
                  ...
                  onClick={() => setNiche(n)}
                  disabled={quotaExhausted}
                  className={`text-xs px-3 py-1.5 rounded-full border transition-all disabled:opacity-40 ${niche === n ? "border-secondary bg-secondary/20 text-secondary font-semibold" : "border-border text-muted-foreground hover:border-secondary/40"}`}
                >
                  {n}
                </button>
              ))}
          ...
          <Button
            onClick={generate}
            disabled={busy || !topic.trim() || quotaExhausted}
          ...
          <div className="pt-1 border-t border-border">
            <div className="flex items-center justify-between mb-2">
              <Label className="flex items-center gap-1.5 text-xs">
                <Lightbulb className="h-3.5 w-3.5 text-secondary" /> Idées suggérées
              </Label>
              <Button
                variant="ghost"
                size="sm"
                className="h-6 text-xs px-2 gap-1"
                onClick={() => fetchIdeas(niche)}
                disabled={ideasBusy}
              >
                ...
                <Card className="p-10 bg-card border-border h-full flex items-center justify-center text-center">
                  <div className="space-y-3">
                    ...
                    <div className="text-muted-foreground text-sm">
                      Votre script viral apparaîtra ici.
                    </div>
                    <div className="text-xs text-muted-foreground/60">
                      TikTok · Instagram · Facebook · SEO
                    </div>
                  </div>
                </Card>
              </motion.div>
              ...
              >
                <Card className="p-10 bg-card border-border flex items-center justify-center">
                  <div className="text-center space-y-3">
                    ...
                    <div className="text-xs text-muted-foreground/60">
                      Analyse des tendances locales · Optimisation virale
                    </div>
                  </div>
                </Card>
              </motion.div>
                ...
                <Card className="bg-card border-border overflow-hidden">
                  ...
                  <Tabs defaultValue="tiktok" className="w-full">
                    <TabsList className="w-full rounded-none border-b border-border bg-muted/30 h-10 px-2">
                      ...
                      <TabsTrigger value="facebook" className="text-xs gap-1">
                        👥 Facebook
                      </TabsTrigger>
                    ...
                    <TabsContent value="tiktok" className="p-5 space-y-3">
                      <Section
                        ...
                        text={script.hook}
                        accent
                        onCopy={() => copySection(script.hook)}
                      />
                      <Section
                        label="CONTENU"
                        text={script.content}
                        onCopy={() => copySection(script.content)}
                      />
                      <Section
                        label="CTA"
                        text={script.cta}
                        accent
                        onCopy={() => copySection(script.cta)}
                      />
                    </TabsContent>
                    <TabsContent value="instagram" className="p-5">
                      <Section
                        ...
                        text={
                          ...
                          "Générez un script pour voir la caption Instagram."
                        }
                    ...
                    <TabsContent value="facebook" className="p-5">
                      <Section
                        label="POST FACEBOOK"
                        text={
                          script.facebook_post ?? "Générez un script pour voir le post Facebook."
                        }
                        onCopy={() => copySection(script.facebook_post ?? "")}
                      />
                    </TabsContent>
                    <TabsContent value="hashtags" className="p-5 space-y-3">
                      <div className="text-[11px] font-bold tracking-widest text-primary mb-2">
                        HASHTAGS OPTIMISÉS
                      </div>
                      <div className="flex flex-wrap gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => copySection((script.hashtags ?? []).join(" "))}
                      >
                    ...
                    <TabsContent value="seo" className="p-5 space-y-3">
                      <Section
                        label="TITRE SEO"
                        text={script.seo_title ?? "—"}
                        onCopy={() => copySection(script.seo_title ?? "")}
                      />
                      <Section
                        label="META DESCRIPTION"
                        text={script.seo_description ?? "—"}
                        onCopy={() => copySection(script.seo_description ?? "")}
                      />
                    </TabsContent>
                  </Tabs>
                </Card>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <div
          className={`text-[11px] font-bold tracking-widest ${accent ? "text-primary" : "text-muted-foreground"}`}
        >
          {label}
        </div>
        {onCopy && (
      ...
      <div className="text-sm leading-relaxed whitespace-pre-wrap rounded-lg bg-muted/50 p-3 border border-border">
        {text}
      </div>
    </div>
  );
