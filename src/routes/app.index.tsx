...
import {
  Flame, Sparkles, Users, FileText, TrendingUp, Share2,
  Calendar, Zap, ArrowRight, CheckCircle2, Circle, Globe,
  BarChart3, Plus
} from "lucide-react";
...
function Dashboard() {
  ...
  const kpis = [
    { label: "Scripts générés", value: stats?.scriptsCount ?? 0, icon: Sparkles, change: "+12%", positive: true },
    { label: "Clients actifs", value: stats?.clientsCount ?? 0, icon: Users, change: "+2", positive: true },
    { label: "Score viral moyen", value: stats?.avgScore ?? 0, icon: TrendingUp, change: "+5pts", positive: true },
    { label: "Tendances live", value: stats?.topTrends.length ?? 0, icon: Flame, change: "Sénégal", positive: true },
  ];
    ...
    >
      <motion.div variants={fadeUp} className="flex items-end justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Voici l'état de votre agence TikTok aujourd'hui.</p>
        </div>
        <Link to="/app/generator">
          <Button className="font-semibold gap-2" style={{ background: "var(--gradient-brand)", color: "var(--primary-foreground)" }}>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map((k) => (
          <Card key={k.label} className="p-5 bg-card/60 backdrop-blur border-border hover:border-primary/40 transition-all group">
        ...
        <motion.div variants={fadeUp} className="lg:col-span-2">
          <Card className="p-5 bg-card border-border h-full">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="font-semibold flex items-center gap-2"><BarChart3 className="h-4 w-4 text-primary" /> Activité cette semaine</div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={weeklyData}>
                ...
                <XAxis dataKey="day" tick={{ fontSize: 11, fill: "oklch(0.70 0.02 260)" }} axisLine={false} tickLine={false} />
                ...
                />
                <Area type="monotone" dataKey="views" stroke="oklch(0.72 0.20 340)" strokeWidth={2} fill="url(#gradViews)" name="Vues" />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
        </motion.div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <motion.div variants={fadeUp} className="lg:col-span-2">
          <Card className="p-5 bg-card border-border">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/app/trends" className="text-xs text-primary hover:underline flex items-center gap-1">
                ...
                {stats?.topTrends.map((t: any) => (
                  <div key={t.id} className="py-3 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge variant="outline" className="border-secondary/50 text-secondary text-[10px]">+{Number(t.growth ?? 0).toFixed(1)}%</Badge>
                      <Badge className="text-[10px]" style={{ background: "var(--gradient-brand)", color: "var(--primary-foreground)" }}>{t.viral_score}</Badge>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <motion.div variants={fadeUp}>
          <Card className="p-5 bg-card border-border h-full">
            ...
            <div className="text-xs text-muted-foreground mb-3">{doneCount}/{onboardingSteps.length} étapes complètes</div>
            ...
            <div className="space-y-2.5">
              {onboardingSteps.map((step) => (
                <div key={step.id} className="flex items-center gap-2.5">
                  ...
                  {step.link && !step.done
                    ? <Link to={step.link} className="text-sm hover:text-primary transition-colors">{step.label}</Link>
