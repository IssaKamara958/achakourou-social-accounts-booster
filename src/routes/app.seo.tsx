...
const samplePages: SeoPageItem[] = [
  {
    ...
    metaDescription: "Découvrez les stratégies IA pour exploser votre audience TikTok au Sénégal. Tendances locales, hooks viraux et hashtags.",
  ...
  },
  {
    ...
    metaDescription: "Les meilleurs hashtags pour maximiser votre portée sur TikTok, Instagram et Facebook en Afrique.",
...
function SeoPage() {
  ...
  async function generateSeo() {
    ...
    if (!canUse("seo")) {
      toast.error(`Quota SEO atteint — ${quota.limit} pages/jour en plan gratuit. Revenez demain ! 🇸🇳`);
      return;
    }
    setGenerating(true);
    ...
    const generated: SeoPageItem = {
      ...
      indexed: false, createdAt: new Date().toISOString().split("T")[0],
    };
    incrementQuota("seo"); refreshQuota();
  ...
  function addManual(e: React.FormEvent) {
    ...
    setPages((prev) => [{ ...newPage, id: Date.now().toString(), indexed: false, createdAt: new Date().toISOString().split("T")[0] }, ...prev]);
    ...
    <motion.div className="space-y-6" initial="hidden" animate="show"
      ...
      <motion.div variants={fadeUp} className="flex items-end justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Générez des pages SEO indexables pour chaque contenu.</p>
        </div>
        ...
        <motion.div variants={fadeUp}>
          <div className="flex items-center gap-3 p-4 rounded-xl border border-destructive/40 bg-destructive/5 text-sm">
            ...
            <div>
              ...
              <span className="text-muted-foreground">{quota.limit} pages SEO/jour en plan gratuit. Renouvellement à minuit. 🇸🇳</span>
            </div>
          </div>
        </motion.div>
      )}
        ...
        <Card className="p-6 bg-card border-border space-y-4">
          ...
          <div className="flex gap-3">
            <Input value={topic} onChange={(e) => setTopic(e.target.value)}
              ...
              className="flex-1" onKeyDown={(e) => e.key === "Enter" && generateSeo()}
              disabled={quotaExhausted} />
            <Button onClick={generateSeo} disabled={generating || quotaExhausted}
              style={!quotaExhausted ? { background: "var(--gradient-brand)", color: "var(--primary-foreground)" } : {}}
              className="font-semibold shrink-0">
              {quotaExhausted ? `Quota atteint (${quota.limit}/j)` : generating ? "Génération…" : "Générer page SEO"}
            </Button>
          </div>
          {!quotaExhausted && (
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{quota.used}/{quota.limit} pages générées aujourd'hui</span>
          ...
          <p className="text-xs text-muted-foreground">
            L'IA génère automatiquement : slug SEO, titre optimisé, meta description, contenu de base avec schema.org.
          </p>
        </Card>
      </motion.div>
            ...
            <Card className="p-6 bg-card border-border border-primary/30">
              <form onSubmit={addManual} className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2"><Label>Titre SEO</Label>
                    <Input value={newPage.title} onChange={(e) => setNewPage({ ...newPage, title: e.target.value })} required /></div>
                  <div className="space-y-2"><Label>Slug URL</Label>
                    <Input value={newPage.slug} onChange={(e) => setNewPage({ ...newPage, slug: e.target.value })} placeholder="mon-article-seo" required /></div>
                </div>
                <div className="space-y-2"><Label>Meta description</Label>
                  <Textarea rows={2} value={newPage.metaDescription} onChange={(e) => setNewPage({ ...newPage, metaDescription: e.target.value })} className="resize-none" /></div>
                <div className="space-y-2"><Label>Contenu</Label>
                  <Textarea rows={4} value={newPage.content} onChange={(e) => setNewPage({ ...newPage, content: e.target.value })} className="resize-none" /></div>
                <div className="flex gap-3">
                  <Button type="submit" style={{ background: "var(--gradient-brand)", color: "var(--primary-foreground)" }}>Créer</Button>
      ...
      <motion.div variants={fadeUp} className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="font-semibold flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /> Pages créées</span>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <Badge variant="outline"
                      ...
                      {page.indexed ? <><CheckCircle2 className="h-3 w-3 mr-0.5" />Indexé</> : "Non indexé"}
                    </Badge>
                  </div>
                  ...
                  <div className="text-xs text-muted-foreground line-clamp-2">{page.metaDescription}</div>
                  <div className="text-[10px] text-muted-foreground mt-1">{page.createdAt}</div>
                </div>
                <div className="flex gap-1 shrink-0">
                  <Button variant="ghost" size="sm" onClick={() => copySlug(page.slug)} className="h-8 w-8 p-0">
                    <Copy className="h-3.5 w-3.5" />
                  </Button>
                  <Button variant={page.indexed ? "outline" : "ghost"} size="sm" onClick={() => toggleIndex(page.id)} className="h-8 text-xs px-2">
