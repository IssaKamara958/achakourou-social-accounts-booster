...
function AppLayout() {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-3">
          <div className="h-10 w-10 rounded-xl flex items-center justify-center font-black text-white"
            style={{ background: "linear-gradient(135deg, oklch(0.72 0.20 340), oklch(0.80 0.15 200))" }}>A</div>
  ...
  const profileName = localStorage.getItem(`profile_name_${user.id}`) || user.email?.split("@")[0] || "Créateur";
    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 flex items-center border-b border-border px-3 gap-2 sticky top-0 z-10 bg-background/80 backdrop-blur-md">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <Link to="/app/settings">
                <Button variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground max-w-[120px] truncate px-2">
                  {profileName}
                </Button>
              </Link>
            </div>
          ...
          <nav className="md:hidden fixed bottom-0 left-0 right-0 z-20 border-t border-border bg-background/95 backdrop-blur-md safe-area-inset">
            <div className="flex items-center justify-around h-16 px-1">
              {mobileNav.map(({ to, icon: Icon, label }) => {
                ...
                return (
                  <Link key={to} to={to} className="flex flex-col items-center gap-0.5 px-2 py-2 min-w-0 flex-1">
                    <div className={`h-7 w-7 rounded-lg flex items-center justify-center transition-all ${active ? "text-white" : "text-muted-foreground"}`}
                      style={active ? { background: "linear-gradient(135deg, oklch(0.72 0.20 340), oklch(0.80 0.15 200))" } : {}}>
                      <Icon className="h-4 w-4" />
                    </div>
                    <span className={`text-[10px] font-medium truncate ${active ? "text-primary" : "text-muted-foreground"}`}>{label}</span>
                  </Link>
                );
              })}
            </div>
