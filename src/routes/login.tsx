        ...
        <div className="text-center">
          <Link to="/" className="inline-flex items-center gap-2">
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] text-muted-foreground uppercase tracking-widest">100% Gratuit 🇸🇳</div>
            </div>
          </Link>
        </div>
        ...
        <Card className="p-6 sm:p-8 bg-card/90 backdrop-blur border-border space-y-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {mode === "signin" ? "Connectez-vous à votre espace créateur." : "Créez votre compte gratuitement."}
            </p>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button type="submit" disabled={busy} className="w-full font-bold h-11"
              style={{ background: "linear-gradient(135deg, oklch(0.72 0.20 340), oklch(0.80 0.15 200))", color: "white" }}>
              {busy ? "Chargement…" : mode === "signin" ? "Se connecter" : "Créer mon compte gratuit"}
            </Button>
          </form>

          {mode === "signup" && (
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              {[["20", "text-primary", "Géné. IA/j"], ["10", "text-secondary", "Posts/j"], ["5", "text-green-400", "SEO/j"]].map(([v, c, l]) => (
          ...
          <button type="button" onClick={() => setMode(mode === "signin" ? "signup" : "signin")}
            ...
            {mode === "signin" ? "Pas encore de compte ? Inscrivez-vous gratuitement →" : "Déjà un compte ? Se connecter"}
          </button>
        </Card>
