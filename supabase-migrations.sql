-- ================================================================
-- ACHAKOUROU BOOSTER AI — Supabase Schema Migration
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='social_accounts' AND column_name='connected') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='posts' AND column_name='hook') THEN
    ALTER TABLE public.posts ADD COLUMN hook text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='posts' AND column_name='cta') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='sync_jobs' AND column_name='payload') THEN
...
CREATE OR REPLACE FUNCTION public.get_sync_jobs(uid uuid)
...
-- ----------------------------------------------------------------
-- 5. Additional tables (trends, clients, generated_scripts, etc.)
-- ----------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.generated_scripts (
  ...
  CONSTRAINT generated_scripts_user_id_fkey FOREIGN KEY (user_id)
    REFERENCES public.profiles (id) ON DELETE CASCADE,
  CONSTRAINT generated_scripts_client_id_fkey FOREIGN KEY (client_id)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='ai_optimization_cache' AND policyname='aoc_all') THEN
    CREATE POLICY aoc_all ON public.ai_optimization_cache
      USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
END$$;

-- ----------------------------------------------------------------
-- 7. Seed: sample trends (only if table is empty)
-- ----------------------------------------------------------------
